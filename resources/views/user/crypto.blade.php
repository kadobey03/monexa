...
<div class="w-full max-w-6xl mx-auto px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{route('mplans')}}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">{{ __('user.crypto.investment_plans') }}</a>
                    </div>
                </li>
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{ __('user.crypto.cryptocurrency') }}</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h1 class="text-4xl font-bold text-gray-800 dark:text-white mb-2 flex items-center">
                    ...
                    <span class="bg-gradient-to-r from-orange-500 to-yellow-500 bg-clip-text text-transparent">{{ __('user.crypto.page_title') }}</span>
                </h1>
                <p class="text-lg text-gray-600 dark:text-gray-300 mb-4">{{ __('user.crypto.page_subtitle') }}</p>
            ...
            <div class="mt-6 lg:mt-0">
                <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-100 dark:border-gray-700">
                    <div class="text-center">
                        ...
                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ __('user.crypto.daily_returns') }}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-gradient-to-r from-orange-50 to-yellow-50 dark:from-gray-800 dark:to-gray-700 rounded-xl p-6 mb-8 border border-orange-100 dark:border-gray-600">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-4 flex items-center">
            ...
            {{ __('user.crypto.market_insights') }}
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="text-center">
                <div class="text-2xl font-bold text-orange-600 dark:text-orange-400">$1.2T</div>
                <div class="text-sm text-gray-600 dark:text-gray-300">{{ __('user.crypto.total_market_cap') }}</div>
            </div>
            <div class="text-center">
                <div class="text-2xl font-bold text-yellow-600 dark:text-yellow-400">24/7</div>
                <div class="text-sm text-gray-600 dark:text-gray-300">{{ __('user.crypto.trading_hours') }}</div>
            </div>
            <div class="text-center">
                <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">10K+</div>
                <div class="text-sm text-gray-600 dark:text-gray-300">{{ __('user.crypto.active_coins') }}</div>
            </div>
            <div class="text-center">
                <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">100M+</div>
                <div class="text-sm text-gray-600 dark:text-gray-300">{{ __('user.crypto.global_users') }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 mb-12">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex items-center justify-between mb-3">
                            <div class="text-2xl font-bold text-orange-600 dark:text-orange-400">
                                ...
                                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">{{ __('user.crypto.minimum') }}</span>
                            </div>
                            <div class="text-right">
                                <div class="text-sm text-gray-500 dark:text-gray-400">{{ __('user.crypto.duration') }}</div>
                                <div class="font-semibold text-gray-800 dark:text-white">{{$plan->expiration}} {{ __('user.crypto.days') }}</div>
                            </div>
                        </div>
                    ...
                    <div class="space-y-3 mb-6">
                        ...
                        <div class="flex items-center justify-between py-2 px-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <div class="flex items-center">
                                ...
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('user.crypto.investment_range') }}</span>
                            </div>
                        ...
                        <div class="flex items-center justify-between py-2 px-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                            <div class="flex items-center">
                                ...
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('user.crypto.return_rate') }}</span>
                            </div>
                        ...
                        <div class="flex items-center justify-between py-2 px-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                            <div class="flex items-center">
                                ...
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('user.crypto.total_expected_return') }}</span>
                            </div>
                        ...
                        <div class="flex items-center justify-between py-2 px-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                            <div class="flex items-center">
                                ...
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('user.crypto.welcome_bonus') }}</span>
                            </div>
                        ...
                        <div data-plan-index="{{ $index }}" data-min-price="{{ $plan->min_price }}" data-max-price="{{ $plan->max_price }}" data-increment="{{ $plan->increment_amount }}" data-expected-return="{{ $plan->expected_return }}">
                            ...
                            <div>
                                <label for="amount-{{$index}}" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    {{ __('user.crypto.investment_amount') }} ({{Auth::user()->currency}})
                                </label>
                                ...
                                <div class="relative">
                                    ...
                                    <input
                                        ...
                                        placeholder="{{ __('user.crypto.enter_amount') }}"
                            ...
                            >
                                <span class="flex items-center justify-center">
                                    ...
                                    {{ __('user.crypto.invest_in_crypto') }}
                                </span>
                            </button>
                        </div>
                    </form>
        ...
        @empty
            ...
            <div class="col-span-full flex flex-col items-center justify-center p-12 bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-100 dark:border-gray-700">
                ...
                <h3 class="text-2xl font-bold text-gray-800 dark:text-white mb-2">{{ __('user.crypto.no_plans_available') }}</h3>
                <p class="text-gray-600 dark:text-gray-400 text-center max-w-md mb-6">
                    {{ __('user.crypto.no_plans_description') }}
                </p>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-8 mb-8 border border-gray-100 dark:border-gray-700">
        <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-6 flex items-center">
            ...
            {{ __('user.crypto.why_choose_crypto_investment') }}
        </h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="flex space-x-4 p-4 bg-orange-50 dark:bg-orange-900/20 rounded-xl">
                ...
                <div>
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">{{ __('user.crypto.high_growth_potential') }}</h4>
                    <p class="text-gray-600 dark:text-gray-400 text-sm">{{ __('user.crypto.growth_description') }}</p>
                </div>
            </div>
            ...
            <div class="flex space-x-4 p-4 bg-purple-50 dark:bg-purple-900/20 rounded-xl">
                ...
                <div>
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">{{ __('user.crypto.twentyfour_trading') }}</h4>
                    <p class="text-gray-600 dark:text-gray-400 text-sm">{{ __('user.crypto.trading_description') }}</p>
                </div>
            </div>
            ...
            <div class="flex space-x-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                ...
                <div>
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">{{ __('user.crypto.global_access') }}</h4>
                    <p class="text-gray-600 dark:text-gray-400 text-sm">{{ __('user.crypto.global_description') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 rounded-xl p-8 mb-8 border border-gray-200 dark:border-gray-600">
        <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-6 text-center">{{ __('user.crypto.our_portfolio') }}</h3>
    ...
    <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-6">
        <div class="flex">
            ...
            <div class="ml-3">
                <h3 class="text-sm font-medium text-yellow-800 dark:text-yellow-200">{{ __('user.crypto.risk_notice_title') }}</h3>
                <div class="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
                    <p>{{ __('user.crypto.risk_notice_description') }}</p>
                </div>
            </div>
        </div>
    </div>
