...
<div class="container mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8" id="referralApp">
    ...
    <div class="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        ...
        <div class="p-4 sm:p-6">
            ...
            <!-- Referrals Tab -->
            <div id="referralsTabContent" style="display: none;">
                <!-- Search and Filter -->
                <div class="mb-6">
                ...
                <div class="lg:hidden space-y-4">
                    ...
                    @if($referrals->count() > 0)
                        ...
                        <div class="bg-gray-800 rounded-lg p-4 border border-gray-700 referral-card" data-name="{{ strtolower($referral->name) }}" data-email="{{ strtolower($referral->email) }}">
    ...
    <!-- Share Modal -->
    <div id="shareModal" class="fixed inset-0 z-50 overflow-y-auto transition-all duration-300 opacity-0" style="display: none;">
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-900 bg-opacity-75 transition-opacity" onclick="referralManager.closeShareModal()"></div>
    ...
    <!-- Toast Notification -->
    <div id="toastNotification" class="fixed bottom-4 right-4 bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg z-50 transition-all duration-300 opacity-0" style="display: none;">
        <div class="flex items-center">
...
class ReferralManager {
    ...
    toggleQRCode() {
        ...
        if (qrSection) {
            qrSection.style.display = this.showQRCode ? 'block' : 'none';
        }
    }
    ...
    setActiveTab(tab) {
        ...
        const referralsContent = document.getElementById('referralsTabContent');

        if (tab === 'overview') {
            if (overviewTab) {
                overviewTab.className = 'px-4 sm:px-6 py-3 sm:py-4 text-sm font-medium transition-colors duration-200 bg-blue-600 text-white';
            }
            if (referralsTab) {
                referralsTab.className = 'px-4 sm:px-6 py-3 sm:py-4 text-sm font-medium transition-colors duration-200 text-gray-400 hover:text-white hover:bg-gray-800';
            }
            if (overviewContent) overviewContent.style.display = 'block';
            if (referralsContent) referralsContent.style.display = 'none';
        } else {
            if (referralsTab) {
                referralsTab.className = 'px-4 sm:px-6 py-3 sm:py-4 text-sm font-medium transition-colors duration-200 bg-blue-600 text-white';
            }
            if (overviewTab) {
