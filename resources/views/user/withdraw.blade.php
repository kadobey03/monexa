...
<div class="w-full max-w-6xl mx-auto px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __('user.withdraw.title') }}</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-2">{{ __('user.withdraw.description') }}</p>
            </div>
            <a href="{{ route('dashboard') }}"
               class="inline-flex items-center gap-2 px-6 py-3 bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-xl font-medium transition-all duration-200 shadow-lg hover:shadow-xl">
                ...
                {{ __('user.withdraw.back_to_dashboard') }}
            </a>
        </div>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                <li class="inline-flex items-center">
                    <a href="{{ route('dashboard') }}" class="inline-flex items-center text-sm text-gray-600 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400">
                        ...
                        {{ __('user.withdraw.home') }}
                    </a>
                </li>
                <li aria-current="page">
                    <div class="flex items-center">
                        <x-heroicon name="chevron-right" class="w-4 h-4 text-gray-400 mx-1" />
                        <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('user.withdraw.withdrawal') }}</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="bg-gray-900 dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-700 dark:border-gray-700 max-w-3xl mx-auto">
            <div class="p-6 border-b border-gray-700 dark:border-gray-700">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-2xl font-bold text-white dark:text-white">{{ $payment_mode }} {{ __('user.withdraw.withdrawal') }}</h2>
                        <p class="text-sm text-gray-300 dark:text-gray-400">{{ __('user.withdraw.complete_request') }}</p>
                    </div>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('completewithdrawal') }}" class="p-6" id="withdrawalForm">
                ...
                <div class="mb-6">
                    <label for="amount" class="block text-sm font-medium text-gray-300 dark:text-gray-300 mb-2">
                        {{ __('user.withdraw.amount_label') }} ({{ Auth::user()->currency }})
                    </label>
                    <div class="relative">
                        ...
                        <input type="number"
                               ...
                               required
                               min="1"
                               placeholder="{{ __('user.withdraw.amount_placeholder') }}"
                    ...
                    <p class="mt-2 text-xs text-gray-400 dark:text-gray-400">
                        {{ __('user.withdraw.available_balance') }}: {{ Auth::user()->currency }}{{ number_format(Auth::user()->account_bal, 2, '.', ',') }}
                    </p>
                </div>
                <!-- Payment Method Specific Fields -->
                @if($payment_mode=="Bank Transfer")
                    <div class="bg-gray-800 dark:bg-gray-700/50 p-4 rounded-xl mb-6">
                        <h3 class="text-lg font-semibold text-white dark:text-white mb-4">{{ __('user.withdraw.bank_details') }}</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="bank_name" class="block text-sm font-medium text-gray-300 dark:text-gray-300 mb-2">
                                    {{ __('user.withdraw.bank_name') }}
                                </label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    ...
                                    <input type="text"
                                           name="bank_name"
                                           id="bank_name"
                                           placeholder="{{ __('user.withdraw.bank_name_placeholder') }}"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text"
                                           name="account_name"
                                           id="account_name"
                                           placeholder="{{ __('user.withdraw.account_name_placeholder') }}"
                                           class="pl-10 block w-full rounded-xl border-gray-600 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-gray-700 dark:bg-gray-700 text-white dark:text-white sm:text-sm py-3"
                                    />
                                </div>
                            </div>
                            ...
                            <div>
                                <label for="account_no" class="block text-sm font-medium text-gray-300 dark:text-gray-300 mb-2">
                                    {{ __('user.withdraw.account_number') }}
                                </label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    ...
                                    <input type="text"
                                           name="account_no"
                                           id="account_no"
                                           placeholder="{{ __('user.withdraw.account_number_placeholder') }}"
                                           class="pl-10 block w-full rounded-xl border-gray-600 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-gray-700 dark:bg-gray-700 text-white dark:text-white sm:text-sm py-3"
                                    />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text"
                                           name="swiftcode"
                                           id="swiftcode"
                                           placeholder="{{ __('user.withdraw.swift_code_placeholder') }}"
                                           class="pl-10 block w-full rounded-xl border-gray-600 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-gray-700 dark:bg-gray-700 text-white dark:text-white sm:text-sm py-3"
                                    />
                                </div>
                            </div>
                ...
                @else
                    <div class="mb-6">
                        <label for="details" class="block text-sm font-medium text-gray-300 dark:text-gray-300 mb-2">
                            {{ $payment_mode }} {{ __('user.withdraw.wallet_address') }}
                        </label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            ...
                            <input type="text"
                                   ...
                                   id="details"
                                   required
                                   placeholder="{{ $payment_mode }} {{ __('user.withdraw.wallet_placeholder') }}"
                                   class="pl-10 block w-full rounded-xl border-gray-600 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 bg-gray-700 dark:bg-gray-700 text-white dark:text-white sm:text-sm py-3"
                            />
                        </div>
                        <p class="mt-2 text-xs text-gray-400 dark:text-gray-400">
                            {{ __('user.withdraw.wallet_warning') }}
                        </p>
                    </div>
                @endif
                ...
                <div class="mt-8">
                    <button type="submit" class="w-full inline-flex justify-center items-center gap-2 py-3 px-5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition-colors duration-200 shadow-lg hover:shadow-xl">
                        ...
                        <span>{{ __('user.withdraw.complete_withdrawal') }}</span>
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="bg-gray-900 dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-700 dark:border-gray-700 max-w-3xl mx-auto mt-8 p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="text-lg font-semibold text-white dark:text-white">{{ __('user.withdraw.withdrawal_info') }}</h3>
            </div>

            <div class="pl-10 space-y-3">
                <div class="flex items-start">
                    <x-heroicon name="check-circle" class="w-4 h-4 text-green-400 dark:text-green-400 mr-2 mt-0.5" />
                    <p class="text-sm text-gray-300 dark:text-gray-300">{{ __('user.withdraw.processing_time') }}</p>
                </div>
                <div class="flex items-start">
                    <x-heroicon name="check-circle" class="w-4 h-4 text-green-400 dark:text-green-400 mr-2 mt-0.5" />
                    <p class="text-sm text-gray-300 dark:text-gray-300">{{ __('user.withdraw.minimum_amount') }}: {{ Auth::user()->currency }}50</p>
                </div>
                <div class="flex items-start">
                    <x-heroicon name="check-circle" class="w-4 h-4 text-green-400 dark:text-green-400 mr-2 mt-0.5" />
                    <p class="text-sm text-gray-300 dark:text-gray-300">{{ __('user.withdraw.fee_info') }} {{ Auth::user()->currency }}5 {{ __('user.withdraw.fee_applied') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div id="modalContent" class="bg-gray-900 dark:bg-gray-900 rounded-2xl shadow-xl transform transition-all max-w-md w-full p-6 z-10 scale-95 opacity-0">
                <div class="text-center">
                    ...
                    <h3 class="text-xl font-semibold text-white dark:text-white mb-2">{{ __('user.withdraw.confirm_withdrawal') }}</h3>
                    <p class="mb-6 text-gray-300 dark:text-gray-400">
                        {{ Auth::user()->currency }}<span id="confirmAmount"></span> {{ __('user.withdraw.confirm_message') }} {{ $payment_mode }} {{ __('user.withdraw.to_account') }}?
                    </p>
