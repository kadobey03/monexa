                    ...
                    <div class="relative" id="notificationsDropdown">
                        <button onclick="toggleNotificationsDropdown()"
                            ...
                            @php
                                ...
                                $recentNotifications = $adminUser ? \App\Models\Notification::where('admin_id', $adminUser->id)->orderBy('created_at', 'desc')->limit(5)->get() : collect();
                            @endphp
                        ...
                        <div id="notificationsDropdownContent"
                             class="absolute right-0 mt-2 w-96 bg-white dark:bg-admin-800 rounded-2xl shadow-elegant dark:shadow-glass-dark border border-admin-200 dark:border-admin-700 max-h-[500px] overflow-hidden z-50 transition opacity-0"
                             style="display: none;">
                            
                            <!-- Header -->
                            <div class="px-6 py-4 border-b border-admin-200 dark:border-admin-700 bg-gradient-to-r from-primary-50 to-primary-100/50 dark:from-admin-900/80 dark:to-admin-800/80">
                            ...
                            <div class="overflow-y-auto max-h-72">
                                @if($recentNotifications->count() > 0)
                                    @foreach($recentNotifications as $notification)
                                        <div class="p-4 border-b border-admin-100 dark:border-admin-700/50 hover:bg-admin-50 dark:hover:bg-admin-700/30 transition-colors group {{ !$notification->is_read ? 'bg-blue-50/50 dark:bg-blue-900/20' : '' }}">
                                            <div class="flex space-x-3">
                                                <div class="flex-shrink-0">
                                                    ...
                                                    ">
                                                        @switch($notification->type)
                                                            @case('deposit')
                                                                <x-heroicon name="trending-up" class="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
                                                                @break
                                                            @case('withdrawal')
                                                                <x-heroicon name="trending-down" class="w-5 h-5 text-rose-600 dark:text-rose-400" />
                                                                @break
                                                            @case('plan_purchase')
                                                                <x-heroicon name="chart-bar" class="w-5 h-5 text-purple-600 dark:text-purple-400" />
                                                                @break
                                                            @case('bot_purchase')
                                                                <x-heroicon name="cpu-chip" class="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                                                                @break
                                                            @case('trade')
                                                                <x-heroicon name="arrows-right-left" class="w-5 h-5 text-amber-600 dark:text-amber-400" />
                                                                @break
                                                            @case('profit')
                                                                <x-heroicon name="currency-dollar" class="w-5 h-5 text-green-600 dark:text-green-400" />
                                                                @break
                                                            @case('login')
                                                                <x-heroicon name="user" class="w-5 h-5 text-blue-600 dark:text-blue-400" />
                                                                @break
                                                            @default
                                                                <x-heroicon name="bell" class="w-5 h-5 text-gray-600 dark:text-gray-400" />
                                                        @endswitch
                                                    </div>
                                                </div>
                                                <div class="flex-1 min-w-0">
                                                    <div class="flex items-center justify-between mb-1">
                                                        <p class="text-sm font-medium text-admin-900 dark:text-admin-100 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                                                            {{ $notification->title }}
                                                        </p>
                                                        <span class="text-xs text-admin-500 dark:text-admin-400">
                                                            {{ $notification->created_at->diffForHumans() }}
                                                        </span>
                                                    </div>
                                                    <p class="text-sm text-admin-600 dark:text-admin-300 line-clamp-2">
                            ...
                            <!-- Action Buttons -->
                            <div class="p-4 bg-admin-50 dark:bg-admin-900/30 border-t border-admin-200 dark:border-admin-700">
                                <div class="flex space-x-3">
    ...
    <script>
        ...
        async function markAllNotificationsRead() {
            try {
                ...
                if (response.ok) {
                    // Reload the page to refresh notification count
