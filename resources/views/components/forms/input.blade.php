@props([
    'name',
    'label' => null,
    'type' => 'text',
    'value' => null,
    'placeholder' => null,
    'required' => false,
    'error' => null,
    'disabled' => false,
    'readonly' => false,
    'hint' => null
])

@php
    $inputId = $name . '-' . uniqid();
    $hasError = !empty($error);
@endphp

<div class="space-y-1">
    @if($label)
        <label for="{{ $inputId }}" class="block text-sm font-medium text-text-primary">
            {{ $label }}
            @if($required)
                <span class="text-error-500">*</span>
            @endif
        </label>
    @endif

    <div class="relative">
        <input
            id="{{ $inputId }}"
            name="{{ $name }}"
            type="{{ $type }}"
            placeholder="{{ $placeholder }}"
            value="{{ $value }}"
            @if($required) required @endif
            @if($disabled) disabled @endif
            @if($readonly) readonly @endif
            class="block w-full px-3 py-2 border rounded-md shadow-sm placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 sm:text-sm
                   {{ $hasError ? 'border-error-300 text-error-900 placeholder-error-300 focus:ring-error-500 focus:border-error-500' : 'border-border' }}
                   {{ $disabled ? 'bg-neutral-50 cursor-not-allowed' : 'bg-white' }}"
            aria-describedby="{{ $hasError ? $inputId . '-error' : ($hint ? $inputId . '-hint' : null) }}"
            aria-invalid="{{ $hasError ? 'true' : 'false' }}"
            {{ $attributes }}
        />
    </div>

    @if($hint && !$hasError)
        <p id="{{ $inputId }}-hint" class="text-sm text-text-secondary">
            {{ $hint }}
        </p>
    @endif

    @if($hasError)
        <p id="{{ $inputId }}-error" class="text-sm text-error-600" role="alert">
            {{ $error }}
        </p>
    @endif
</div>