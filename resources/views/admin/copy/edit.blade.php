    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <nav class="flex items-center text-sm text-gray-600 dark:text-gray-400 mb-2">
                ...
                <span class="text-gray-900 dark:text-white">{{ __('admin.copy.edit_expert') }}</span>
            </nav>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $title }}</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-2">{{ __('admin.copy.update_expert_trader_info') }}</p>
        </div>
    ...
    <div class="max-w-4xl">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('admin.copy.update_details_for') }} {{ $expert->name }}</p>
            </div>
            ...
            <form action="{{ route('admin.copy.update', $expert->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
                    ...
                    <div class="space-y-6">
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="flex-1">
                                    <input type="file" 
                                           ...
                                           class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                                    <p class="text-xs text-gray-500 mt-1">{{ __('admin.copy.photo_requirements_edit') }}</p>
                                </div>
                            </div>
                            @error('photo')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <textarea id="description" 
                                      ...
                                      rows="4"
                                      class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                                      placeholder="{{ __('admin.copy.description_placeholder') }}">{{ old('description', $expert->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="status" 
                                ...
                                <option value="active" {{ old('status', $expert->status) == 'active' ? 'selected' : '' }}>{{ __('admin.copy.active') }}</option>
                                <option value="inactive" {{ old('status', $expert->status) == 'inactive' ? 'selected' : '' }}>{{ __('admin.copy.inactive') }}</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                ...
                <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('admin.copy.current_statistics') }}</h3>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                            <div class="text-sm text-gray-600 dark:text-gray-400">{{ __('admin.copy.total_copiers') }}</div>
                            <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ $expert->copiers()->count() }}</div>
                        </div>
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                            <div class="text-sm text-gray-600 dark:text-gray-400">{{ __('admin.copy.copy_volume') }}</div>
                ...
                <div class="flex items-center gap-4 pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
                    <button type="submit"
                        ...
                        {{ __('admin.copy.update_expert_trader') }}
                    </button>
                    <a href="{{ route('admin.copy.index') }}"
