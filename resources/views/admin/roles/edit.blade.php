...
<div class="space-y-6" x-data="roleEditManager()">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-admin-600 dark:text-admin-400">Rol bilgilerini ve izinlerini güncelle</p>
                </div>
            </div>
    ...
    <form action="{{ route('admin.roles.update', $role) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" 
                              ...
                              placeholder="Bu rolün yetkilerini ve sorumluluklarını açıklayın...">{{ old('description', $role->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <select id="parent_role_id" 
                        ...
                        @foreach($parentRoles as $parentRole)
                            <option value="{{ $parentRole->id }}" {{ old('parent_role_id', $role->parent_role_id) == $parentRole->id ? 'selected' : '' }}>
                                {{ str_repeat('- ', $parentRole->hierarchy_level) }}{{ $parentRole->display_name }} (Seviye {{ $parentRole->hierarchy_level }})
                            </option>
                        @endforeach
                    </select>
        ...
        <!-- Current Statistics -->
        <div class="bg-white dark:bg-admin-800 rounded-2xl shadow-elegant dark:shadow-glass-dark p-6 border border-admin-200 dark:border-admin-700">
            <h2 class="text-lg font-semibold text-admin-900 dark:text-white mb-6">Mevcut İstatistikler</h2>
            
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div class="text-center">
                    <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $role->admins->count() }}</div>
                    <div class="text-sm text-admin-600 dark:text-admin-400">Admin Sayısı</div>
                </div>
                <div class="text-center">
                    <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $role->permissions->count() }}</div>
                    <div class="text-sm text-admin-600 dark:text-admin-400">İzin Sayısı</div>
                </div>
                <div class="text-center">
                    <div class="text-2xl font-bold text-amber-600 dark:text-amber-400">{{ $role->childRoles->count() }}</div>
                    <div class="text-sm text-admin-600 dark:text-admin-400">Alt Rol Sayısı</div>
                </div>
                <div class="text-center">
                    <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ $role->hierarchy_level }}</div>
                    <div class="text-sm text-admin-600 dark:text-admin-400">Hiyerarşi Seviyesi</div>
                </div>
            </div>
        </div>
