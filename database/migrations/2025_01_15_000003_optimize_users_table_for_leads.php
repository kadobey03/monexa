...
return new class extends Migration
{
    public function up()
    {
        Schema::table('users', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('users', 'contact_count') && Schema::hasColumn('users', 'contact_history')) {
                $table->integer('contact_count')->default(0)->after('contact_history');
            } elseif (!Schema::hasColumn('users', 'contact_count')) {
            ...
            if (!Schema::hasColumn('users', 'lead_priority') && Schema::hasColumn('users', 'lead_score')) {
                $table->enum('lead_priority', ['low', 'medium', 'high', 'urgent'])->default('medium')->after('lead_score');
            } elseif (!Schema::hasColumn('users', 'lead_priority')) {
            ...
            if (Schema::hasColumn('users', 'assign_to') && Schema::hasColumn('users', 'account_type')) {
            ...
            if (Schema::hasColumn('users', 'country') && Schema::hasColumn('users', 'account_type')) {
            ...
            if (Schema::hasColumn('users', 'lead_score') && Schema::hasColumn('users', 'account_type')) {
            ...
            if (Schema::hasColumn('users', 'account_type')) {
                ...
                if (Schema::hasColumn('users', 'estimated_value')) {
                    $table->index(['estimated_value', 'account_type'], 'idx_estimated_value_account');
                }
            }

            // Composite indexes for complex lead queries
            if (Schema::hasColumn('users', 'assign_to') && Schema::hasColumn('users', 'lead_status_id')) {
                $table->index(['assign_to', 'lead_status_id', 'created_at'], 'idx_assign_status_created');
            }
            if (Schema::hasColumn('users', 'country') && Schema::hasColumn('users', 'lead_source') && Schema::hasColumn('users', 'account_type')) {
                $table->index(['country', 'lead_source', 'account_type'], 'idx_country_source_account');
            }
            if (Schema::hasColumn('users', 'lead_priority') && Schema::hasColumn('users', 'lead_score') && Schema::hasColumn('users', 'account_type')) {
